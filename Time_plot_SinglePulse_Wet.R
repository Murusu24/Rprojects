...
WCM<-plot_node %>%
  ggplot(aes(Time)) +
  scale_y_continuous(limits = c(0.185,0.26))+
  scale_x_continuous(limits = c(0,300), breaks = seq(0,300, 100))+ 
  geom_line(aes(y= Moisture, colour = as.factor(Depth)))+
  geom_hline(yintercept=0.242, linetype="solid", color = "black")+
  annotate("text", x=200, y=0.247, label="Initial")+
  ylab("Theta [-]")+
...
WCF<-plot_node %>%
  ...
  scale_y_continuous()+
  scale_x_continuous(limits = c(0,300), breaks = seq(0,300, 100))+ 
  geom_hline(yintercept=0.032, linetype="solid", color = "black")+
  annotate("text", x=100, y=0.034, label="Initial")+
...
WFM<-plot_node %>%
  ...
  geom_hline(yintercept=-0.185, linetype="solid", color = "black")+
  annotate("text", x=150, y=-0.22, label="Initial")+
  ylab("v [mm/hour]")+
...
WFF<-plot_node %>%
  ...
  geom_hline(yintercept=-7, linetype="solid", color = "black")+
  annotate("text", x=150, y=-6.5, label="Initial")+
  ylab("v [mm/hour]")+
